...
function get_case_study_category_by_id($id, $output = 'array')
{
    $category_id = CaseStudy::find($id)->categories_id;
    $cat_list = [];
    $cat_list_string = '';
    $cat_list_slug = '';
    ...
    foreach ($category_id as $key => $data) {
        $separator = $key != 0 ? ', ' : '';
        $cat_item = CaseStudyCategory::find($data);
        if (!empty($cat_item)){
...
function get_attachment_image_by_id($id, $size = null, $default = false)
{
    ...
    if (!empty($image_details)) {
        ...
        $return_val['img_alt'] = !empty($image_details->alt) ? $image_details->alt : 'Go4database image'.$image_details->id;;
...
function render_image_by_attachment_id($id, $class = null, $size = 'full')
{
    $class_list = !empty($class) ? 'class="' . $class . '"' : '';

    // 👉 Default image path
    $default_image = asset('assets/uploads/no-image.png');; // change path if needed

    if (empty($id)) {
        return '<img src="' . $default_image . '" ' . $class_list . ' alt="default image"/>';
    }

    $image_details = get_attachment_image_by_id($id, $size);

    if (!empty($image_details)) {
        return '<img src="' . $image_details['img_url'] . '" ' . $class_list . ' alt="' . $image_details['img_alt'] . '"/>';
    }
...
function render_logo_image_markup_by_attachment_id($id, $class = null, $size = 'full')
{
    if (empty($id)) return '';
    $output = '';
    ...
    if (!empty($image_details)) {
        $class_list = !empty($class) ? 'class="' . $class . '"' : '';
        $output = '<img src="' . $image_details['img_url'] . '" ' . $class_list . ' alt="'.$image_details['img_alt'].'" width="auto" height="34" loading="eager"/>';
    }
    return $output;
}
...
function get_attachment_image_url($id, $size = 'full')
{
    if (empty($id)) {
        return '';
    }
    ...
    return !empty($image_details)
        ? $image_details['img_url']
        : '';
}
...
function get_blog_category_by_id($id,$type = '',$class = ''){
    ...
    if (!empty($blog_cat)){
        ...
        if ($type == 'link' ){ 
            $return_val = '<a class="'.$class.'" href="'.route('frontend.blog.category',['any' => Str::slug($blog_cat->name) ]).'">'.$blog_cat->name.'</a>';
        }
    }
...
function get_service_category_by_id($id,$type = ''){
    ...
    if (!empty($blog_cat)){
        $return_val = $blog_cat->name;
        if ($type == 'link' ){
            $return_val = '<a href="'.route('frontend.b2c.category',['category_slug' => $blog_cat->slug]).'">'.$blog_cat->name.'</a>';
        }
    }
...
if (!function_exists('fa_class_to_svg')) {
    function fa_class_to_svg($faClass) {
        $iconMap = [
            'fab fa-twitter-square' => '<svg width="15" height="15" viewBox="0 0 24 24" fill="#0b132a"><path d="M23 5.9c-.8.4-1.7.6-2.6.8.9-.6 1.6-1.5 2-2.5-.9.5-1.9.9-2.9 1.1A4.5 4.5 0 0 0 12 8.5c0 .4 0 .7.1 1A12.8 12.8 0 0 1 2.9 4.8a4.5 4.5 0 0 0 1.4 6 4.4 4.4 0 0 1-2-.6v.1c0 2.2 1.6 4 3.6 4.4a4.5 4.5 0 0 1-2 .1 4.5 4.5 0 0 0 4.2 3.1A9 9 0 0 1 1 19.5 12.7 12.7 0 0 0 7.9 21.5c8.3 0 12.8-6.9 12.8-12.8v-.6c.9-.6 1.6-1.4 2.3-2.3Z"></path></svg>',
            'fab fa-facebook' => '<svg width="15" height="15" viewBox="0 0 24 24" fill="#0b132a"><path d="M14 9V6.5c0-.8.2-1.3 1.3-1.3H17V2h-2.8C11.7 2 10 3.6 10 6.3V9H8v3.5h2V22h3.5v-9.5H16l.5-3.5H14Z"></path></svg>',
            'fab fa-instagram' => '<svg width="13" height="13" viewBox="0 0 24 24" fill="#0b132a"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.051.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" /></svg>',
            'fab fa-linkedin' => '<svg width="15" height="15" viewBox="0 0 24 24" fill="#0b132a"><path d="M4.98 3.5a2.5 2.5 0 1 1 0 5 2.5 2.5 0 0 1 0-5ZM3 9h4v12H3zM9 9h3.8v1.7h.05c.53-1 1.83-2 3.77-2 4 0 4.4 2.6 4.4 6V21h-4v-5.3c0-1.27-.02-2.9-1.77-2.9-1.78 0-2.05 1.4-2.05 2.8V21H9Z"></path></svg>',
        ];

        return $iconMap[$faClass] ?? '<i class="' . $faClass . '"></i>';
    }
}
...
ITEM;
   ...
   /* public function blog_page(Request $request)
    { 
        ...
        // echo $request->author;die;
        if(isset($request->author) && $request->author!='')
        { 
            $all_blogs = Blog::where(['lang'=> $lang , 'status' => 'publish','author'=>$request->author])->orderBy('id', 'desc')->paginate(get_static_option('blog_page_item')); 
        }
        else{
...
{ 
    $default_lang = Language::where('default', 1)->first();
    $lang = session()->get('lang') ?? $default_lang->slug;

    // Filter by author if present
    if (!empty($request->author)) {
        $all_blogs = Blog::where([
            'lang' => $lang,
            'status' => 'publish',
        ...
        // Force relative path with author parameter
        $all_blogs->appends(['author' => $request->author]);
        $all_blogs->withPath('/blog');
    } else {
        $all_blogs = Blog::where([
            'lang' => $lang,
            'status' => 'publish'
...
public function blog_author_page($author)
{
   
   
   $authorSlug = $author; // e.g. "ashwini-bochkeri"

// convert slug back to original format for query
$authorName = Str::title(str_replace('-', ' ', $authorSlug));
            $default_lang = Language::where('default', 1)->first();
            $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
            $all_blogs = Blog::where(['author' => $authorName, 'lang' => $lang,'status' => 'publish'])->orderBy('id', 'desc')->paginate(get_static_option('blog_page_item'));
        
            $all_recent_blogs = Blog::where(['lang' => $lang,'status' => 'publish'])->orderBy('id', 'desc')->take(get_static_option('blog_page_recent_post_widget_item'))->get();
            $all_category = BlogCategory::where(['status' => 'publish', 'lang' => $lang])->orderBy('id', 'desc')->get();
            $author_name = Blog::where(['author' => $authorName, 'status' => 'publish'])->first()->name;
            $blogauthor = Blog::where(['author' => $authorName, 'status' => 'publish'])->first();
            return view('frontend.pages.blog.blog-category')->with([
                'all_blogs' => $all_blogs,
                'all_categories' => $all_category,
 ...
 public function author_list()
    {
           ...
           $all_author = Author::where([
            'lang' => $lang,
            'status' => 'publish'
        ])
        ->orderBy('id', 'desc')->get();
   
            return view('frontend.pages.author.all-author')->with([
                'all_authors' => $all_author,
                  'lang' => $lang,
            ]);
    
    }
 public function author_single_page($slug)
    {
    
            $default_lang = Language::where('default', 1)->first();
            $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
            //$blogauthor = Author::where(['slug' => $slug, 'status' => 'publish'])->first();
            $blogauthor = Author::where([
    'slug' => $slug,
    'status' => 'publish'
])->firstOrFail();
  ...
  // dd($authorName);
    ...
    $all_blogs = Blog::where(['author_id' => $blogauthor->id, 'lang' => $lang,'status' => 'publish'])->orderBy('id', 'desc')->paginate(get_static_option('blog_page_item'));
          $all_author = Author::where('id', '!=', $blogauthor->id)->where('lang', $lang)->where('status', 'publish')->orderBy('id', 'desc')->get();
            return view('frontend.pages.blog.blog-author')->with([
                'all_blogs' => $all_blogs,
    ...
    public function blog_single_page($slug)
    {
       ...
       // dd($blogCat);
        if($blogCat)
        {
            ...
            $all_category = BlogCategory::where(['status' => 'publish', 'lang' => $lang])->orderBy('id', 'desc')->get();
            $category_name = BlogCategory::where(['id' => $id, 'status' => 'publish'])->first()->name;
            $blogcat = BlogCategory::where(['id' => $id, 'status' => 'publish'])->first();
            return view('frontend.pages.blog.blog-category')->with([
                'all_blogs' => $all_blogs,
                'all_categories' => $all_category,
                'category_name' => $category_name,
        ...
        else
        {
            ...
            $all_work = Works::where(['lang' => $lang])->orderBy('id', 'desc')->take(4)->get();
     
            //echo "<pre>"; print_r(json_decode($blog_post)->meta_tags); die;

    // Generate Table of Contents
            $toc = $this->generateTOC($blog_post->content);
           // $Visitors = $blog_post->total_visitors+1;
            //$Visitor = array('total_visitors'=>$Visitors);
            //Blog::where(['id' => $blog_post->id])->update($Visitor);
           
           // ✅ FIXED VISITOR COUNT
$blog_post->timestamps = false;
$blog_post->increment('total_visitors');
...
if (!empty($matches[2])) {
    foreach ($matches[2] as $index => $headingText) {
        $tag = $matches[1][$index]; // h1, h2, h3
        //$anchor = Str::slug(strip_tags($headingText)); // creates URL-friendly slug
// Clean heading text
            $cleanText = html_entity_decode(strip_tags($headingText)); // decode &nbsp;
            $cleanText = str_replace("\u{00A0}", ' ', $cleanText);     // remove non-breaking spaces
            $anchor = Str::slug($cleanText); // slugify
        $toc[] = [
            'title' => strip_tags($cleanText),
            'anchor' => $anchor
        ];

        // Build new heading with ID
        $newHeading = "<{$tag} id=\"{$anchor}\" class=\"anchor-offset\">{$headingText}</{$tag}>";

        // Replace the exact original heading
        $content = str_replace($matches[0][$index], $newHeading, $content);
    }
}
    ...
    public function services_single_page($slug)
    {
     ...
     $serviceCat = ServiceCategory::whereRaw("LOWER(REPLACE(name, ' ', '-')) = ?", [$slug])->first();
     $default_lang = Language::where('default', 1)->first();
     $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
    
     $all_serviceCat = ServiceCategory::where('lang', $lang)->get();
        if($serviceCat)
        {
        $id=$serviceCat->id;
        ...
        // dd($category_name);
          $service_item = Services::where(['lang' => $lang, 'status' => 'publish'])->where('categories_id', 'LIKE', '%' . $id . '%')->paginate(12);
        return view('frontend.pages.service.service-category')->with(['service_items' => $service_item, 'category_name' => $category_name,'service_category' => $category]);
        }else {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        ...
        $all_testimonial = Testimonial::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->get();
        return view('frontend.pages.service.service-single')->with(['service_item' => $service_item, 'all_testimonial' =>$all_testimonial, 'all_serviceCat' =>$all_serviceCat, 'price_plan' => $price_plan]);
        }
    }

    public function category_wise_services_page($any)
    {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
    ...
    $workCat = WorksCategory::whereRaw("LOWER(REPLACE(name, ' ', '-')) = ?", [$slug])->first();
    ...
    $id=$workCat->id;
      $category = WorksCategory::findOrFail($id);
        ...
        return view('frontend.pages.work.work-category')->with(['all_work' => $all_works, 'category_name' => $category_name,'work_category' => $category]);
...
$all_testimonial = Testimonial::where('lang', $lang)->orderBy('id', 'desc')->take(get_static_option('about_page_testimonial_item'))->get();
        return view('frontend.pages.work.work-single')->with(['work_item' => $work_item,'work_category' => null, 'related_works' => $all_works,  'all_brand_logo' => $all_brand_logo,'all_testimonial' => $all_testimonial]);
        }
    }
    ...
    public function case_study_page()
    { 
        $default_lang = Language::where('default', 1)->first();
       
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
         
        $all_work = CaseStudy::where(['lang' => $lang])->orderBy('id', 'desc')->paginate(12);
        
        $all_contain_cat = [];
        foreach ($all_work as $work) {
            $all_contain_cat[] = $work->categories_id;
        }
        $all_work_category = CaseStudyCategory::find(array_unique(array_flatten($all_contain_cat)));
        // echo '<pre>';
        // print_r(json_decode($all_work_category));
        // die;
        return view('frontend.pages.case-study.case-study')->with(['all_work' => $all_work, 'all_work_category' => $all_work_category]);
    }

    
    

    public function case_study_single_page($slug)
    {
        $work_item = CaseStudy::where('slug', $slug)->first();
        if (empty($work_item)){
            abort(404);
        }
        $all_works = [];
        $all_related_works = [];
        foreach ($work_item->categories_id as $cat) {
            $all_by_cat = CaseStudy::where(['lang' => get_user_lang()])->where('categories_id', 'LIKE', '%' . $work_item->$cat . '%')->take(6)->get();
            for ($i = 0; $i < count($all_by_cat); $i++) {
                array_push($all_works, $all_by_cat[$i]);
            }
        }
        array_unique($all_works);
        return view('frontend.pages.case-study.case-study-single')->with(['work_item' => $work_item, 'related_works' => $all_works]);
    }
        ...
        public function author_page()
    {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
        $all_author = Author::where(['lang' => $lang, 'status' => 'publish'])->get();
        return view('frontend.pages.author-page')->with([
            'all_author' => $all_author
        ]);
    }
    ...
    { 
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
       $all_price_plan = PricePlan::where([
    'lang' => $lang,
    'categories_id' => 9,
    ...
    public function email_verifier_page()
    {
        ...
        $ev_items = EmailVerifierItem::where(['lang' => $lang, 'status' => 'publish'])
            ->orderBy('sr_order')->get()->groupBy('section');

        // Admin can point this page's FAQ at any Faq category; otherwise the
        // page falls back to the questions built into the blade file.
        $faq_category_id = get_static_option('ev_' . $lang . '_faq_category_id');
        $ev_faqs = !empty($faq_category_id)
            ? Faq::where(['lang' => $lang, 'status' => 'publish', 'category_id' => $faq_category_id])->get()
            : collect();

        return view('frontend.pages.email-verifier')->with([
            'all_testimonial' => $all_testimonial,
            'ev_items' => $ev_items,
            'ev_faqs' => $ev_faqs,
            'ev' => $this->email_verifier_content($lang),
        ]);
    }
    ...
    public function jobs_category($any)
    {
        dd($any);
    $jobCat = JobsCategory::where('slug', $slug)->first();
        $id=$jobCat->id;
    ...
    public function jobs_single($slug)
    {
     ...
     $jobCat = JobsCategory::where('id', $job->category_id)->first();
 
        $all_job_category = JobsCategory::where(['status' => 'publish', 'lang' => $lang])->get();
        return view('frontend.pages.jobs.jobs-single')->with([
            'job' => $job,
            'category_name' => $jobCat->title,
            'all_job_category' => $all_job_category,
...
//****************B2C New route******************************//
    ...
    public function list()
    {
        $default_lang = Language::where('default', 1)->first();
        $lang = !empty(session()->get('lang')) ? session()->get('lang') : $default_lang->slug;
      $list_item = B2BList::where('lang', $lang)->pluck('slug')
    ->map(function ($slug) {
        return ltrim($slug, '/');
    })
    ->toArray();
        $category_item = ListCategory::where('lang', $lang)->paginate(12);
        return view('frontend.pages.list.list')->with(['all_list' => $category_item,'allowedSlugs' => $list_item]);
    }
     
public function list_single($service_slug)
{

    $default_lang = Language::where('default', 1)->first();

    $lang = !empty(session()->get('lang'))
        ? session()->get('lang')
        : $default_lang->slug;

    $service_category = ListCategory::where([
        'status' => 'publish',
        'lang' => $lang
    ])->get();
    
    $all_serviceCat = ListCategory::where('lang', $lang)->get();
    // FIRST get service items
    
    $service_item = B2BList::where('slug', '/'.$service_slug)->first();
    $list_item = B2BList::where('slug', '/'.$service_slug)->first();
     if (empty($service_item)) {
            return redirect_404_page();
        }
    $industry = ListCategory::find($service_item->categories_id);
     if (empty($industry)) {
            return redirect_404_page();
        }
// Related B2BList
$relatedServices = B2BList::where('id', '!=', $service_item->id)
    ->where('categories_id', $service_item->categories_id)
    ->latest()
    ->limit(15)
    ->get();

    // THEN use it
    $price_plan = !empty($service_item->price_plan)
        ? PricePlan::find(unserialize($service_item->price_plan))
        : '';
 $relatedBlogs = Blog::where('lang', $lang)
    ->where('status', 'publish')
    ->get()
    ->filter(function ($blog) use ($service_item) {

        $categories = @unserialize($blog->categories_id);

        if (!is_array($categories)) {
            return false;
        }

        return in_array((string)$service_item->categories_id, $categories);
    })
    ->take(6);
    //dd($list_item);
    $all_testimonial = Testimonial::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->get();
    return view('frontend.pages.list.list-single')->with([
        'service_item' => $service_item,
        'list_item' => $list_item,
        'list_category' => $service_category,
        'price_plan' => $price_plan,
        'all_serviceCat' =>$all_serviceCat, 
    ...
    public function b2c_category($category_slug)
{
    $default_lang = Language::where('default', 1)->first();
    ...
    $service_item = ServiceSubcategory::where('category_id', $categoryId)
        ->paginate(6);

    return view('frontend.pages.service.service-category')->with([
        'service_items' => $service_item,
        'category_name' => $category_name,
        'category_slug' => $category_slug
    ]);
}
public function b2c_child_category($category_slug, $child_category_slug)
{
    ...
    // SUBCATEGORY
    $subcategory = ServiceSubcategory::where('slug', $child_category_slug)
        ->firstOrFail();
    ...
    // SERVICES
    $service_item = Services::where([
        'categories_id' => $categoryId,
        'subcategories_id' => $subId,
    ])->paginate(6);

    return view('frontend.pages.service.service-subcategory')->with([
        'service_items' => $service_item,
        'category_name' => $category_name,
        'category_slug' => $category_slug,
...
public function b2b_child_category($category_slug, $child_category_slug, $title)
{
//dd($category_slug);
$service = Services::where('slug', $title)->first();

    if ($service) {
        return $this->b2c_service_single(
            $category_slug,
            $child_category_slug,
            $title
        );
...
public function b2c_service_single($category_slug, $child_category_slug, $service_slug)
{
    ...
    $service_category = ServiceCategory::where([
        ...
        $all_serviceCat = ServiceCategory::where('lang', $lang)->get();
    // FIRST get service item
    $service_item = Services::where('slug', $service_slug)->first();
    if (empty($service_item)) {
        abort(404);
    }
...
$all_testimonial = Testimonial::where(['lang' => $lang, 'status' => 'publish'])->orderBy('id', 'desc')->get();
    return view('frontend.pages.service.service-single')->with([
        ...
        'subcategory_slug' => $child_category_slug,
        'price_plan' => $price_plan,
        'all_serviceCat' =>$all_serviceCat, 
        'all_testimonial' =>$all_testimonial, 
    ]);
    ...
    public function b2b_service_single($category_slug, $child_category_slug,$title, $service_slug)
{
    $default_lang = Language::where('default', 1)->first();
...
public function submit(Request $request)
{
    $request->validate([
        ...
        'business_details' => 'nullable',
    ]);

    Mail::send('mail.lead', [
        'data' => $request->all()
    ], function ($message) {
       // $message->to('connect@go4database.com')
        $message->to('success@go4database.com')
...
public function submitSample(Request $request)
{
    $request->validate([
        ...
        'sample-phone'   => 'required',
        'sample-message' => 'required',
    ]);

    Mail::send('mail.sample', [
        'data' => $request->all()
    ], function ($message) use ($request) {

        $message->to('success@go4database.com')
                ->replyTo(request('business-email'))
                ->subject('New Sample Request Lead');
    });

    return back()->with('sample-success', 'Thank you! Your request has been submitted.');
}

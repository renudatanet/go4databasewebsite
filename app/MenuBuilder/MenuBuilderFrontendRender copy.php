...
class MenuBuilderFrontendRender
{
    ...
    private function render_menu_item($menu_item, int $page_id, $default_lang)
    {
        ...
        $menutitle =  property_exists($menu_item,'menutitle') ? $menu_item->menutitle : '';
        ...
        if ($ptype === 'custom'){
            ...
            $title = $pname;
            if(isset($menu_item->menutitle) && $menu_item->menutitle!='')
            {
            ...
            if (!empty($matches[0])){
                ...
                if ($instance->enable()){
                    ...
                    $output .= $this->get_anchor_markup($Menu_Title,[

...
class AboutSectionStyleEight extends PageBuilderBase
{
    /**
     * preview_image
    ...
    public function admin_render()
    {
        ...
        foreach ($all_languages as $key => $lang) {
            ...
            $output .= Image::get([
                ...
                'dimensions' => '320x720px'
            ]);
            $output .= Image::get([
                'name' => 'video_image_'.$lang->slug,
                'label' => __('Video Image'),
                'value' => $widget_saved_values['video_image_' . $lang->slug] ?? null,
                'dimensions' => '460x330px'
            ]);
            $output .= Text::get([
    ...
    public function frontend_render(): string
    {
        ...
        $video_image = render_image_markup_by_attachment_id($settings['video_image_'.$current_lang],'','full');

        $button_markup = '';
        if (!empty($button_url) && !empty($button_text)){
            $button_icon = $button_icon ? '<i class="'.$button_icon.'"></i>' : '';
            $button_markup .= ' <div class="btn-wrapper"><a href="'.$button_url.'" class="industry-btn black">'.$button_text.$button_icon.'</a></div>';
        }

        $left_content_markup = '';
        if (!empty($left_image)){
            $shape_url = asset('assets/frontend/img/shape/07.png');
            $vdo_image_btn = '';
            if (!empty($video_url)){
                $vdo_image_btn = ' <div class="hover"><a class="mfp-iframe  vdo-btn" href="'.$video_url.'"><i class="fas fa-play"></i></a> </div>';
            }
            $experience_markup = '';
...
<div class="left-content-wrap">
        <img src="{$shape_url}" alt="" class="shape">
...
HTML;
        ...
        return <<<HTML
<div class="industry-about-area " data-padding-top="{$padding_top}" data-padding-bottom="{$padding_bottom}">
       <div class="container">
        <div class="row">
            <div class="col-lg-6">
               {$left_content_markup}

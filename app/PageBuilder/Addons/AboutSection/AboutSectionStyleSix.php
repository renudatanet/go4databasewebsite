...
class AboutSectionStyleSix extends PageBuilderBase
{
    /**
     * preview_image
    ...
    public function admin_render()
    {
        ...
        foreach ($all_languages as $key => $lang) {
            ...
            $output .= Image::get([
                'name' => 'left_bottom_image_'.$lang->slug,
                'label' => __('Left Bottom Image'),
                'value' => $widget_saved_values['left_bottom_image_' . $lang->slug] ?? null,
                'dimensions' => '350x250px'
            ]);
    ...
    public function frontend_render(): string
    {
        ...
        $left_bottom_image = render_image_markup_by_attachment_id($settings['left_bottom_image_'.$current_lang],'','full');

        $button_markup = '';
        if (!empty($button_url) && !empty($button_text)){
            $button_markup .= ' <div class="btn-wrapper"><a href="'.$button_url.'" class="boxed-btn lawyer-home">'.$button_text.'</a></div>';
        }
...
HTML;
        ...
        return <<<HTML
<div class="lawyer-about-area lawyer-home" data-padding-top="{$padding_top}" data-padding-bottom="{$padding_bottom}">
       <div class="container">
        <div class="row">
            <div class="col-lg-6">
               {$right_content_markup}

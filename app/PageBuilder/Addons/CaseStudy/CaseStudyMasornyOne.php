...
class CaseStudyMasornyOne extends PageBuilderBase
{
    ...
    {
       return 'case-study/masonry-01.png';
    }
    ...
    public function admin_render()
    {
        ...
        $output .= Select::get([
            ...
            'options' => [
                '' => __('Style 01'),
                'cleaning-home' => __('Style 02'),
                'dagency-home' => __('Style 03'),
            ],
    ...
    public function frontend_render()
    {
        ...
        if(!empty($items) && !empty($pagination_status)){
        ...
        if (!empty($categories)){
          $all_case_study_by_category =  $all_case_study->filter(function ($item) use ($categories){
           ...
           $all_contain_cat = $all_case_study_by_category->map(function ($item) { return $item->categories_id;})->toArray();
        if (!empty($categories)){
            $all_work_category = WorksCategory::whereIn('id',array_unique(array_flatten($all_contain_cat)))->get();
        }else{
            $all_work_category = WorksCategory::paginate();
        }
...
<div class="col-lg-4 col-md-4 col-sm-6 masonry-item {$filter_slug}">
    <div class="single-case-studies-item {$theme}">
        <div class="thumb">
            {$image_markup}
        </div>
        <div class="content">
...
HTML;
 ...
 <ul class="case-studies-menu style-01 {$theme}">
...
<div class="row justify-content-center">
    <div class="col-lg-12">
        <div class="section-title desktop-center {$theme} margin-bottom-60 {$section_title_alignment}">
            {$section_subtitle_markup}
...
HTML;
        ...
        return <<<HTML
<div class="case-study-addon-wrapper" data-padding-top="{$padding_top}" data-padding-bottom="{$padding_bottom}">
    <div class="container">
        {$section_title_markup}
        <div class="row">
              <div class="col-lg-12">
...
HTML;
    ...
    {
        return __('Case Study Masonry: 01');
    }
}
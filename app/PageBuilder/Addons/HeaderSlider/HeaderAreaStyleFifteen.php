...
class HeaderAreaStyleFifteen extends PageBuilderBase
{
    use RepeaterHelper;
    /**
     * preview_image
    ...
    {
        return 'header-area-style-15.png';
    }
    ...
    private function render_slider_markup(int $index = null): string
    {
        ...
        if (!empty($button_one_text) && !empty($button_one_url)){
            $button_markup .= '<div class="btn-wrapper desktop-left"> <a href="'.$button_one_url.'" class="boxed-btn">'.$button_one_text.'</a>  </div>';
        }
        if (!empty($button_two_text) && !empty($button_two_url)){
            $icon_markup =  '<i class="fas fa-play"></i>' ;
            $button_markup .= '<div class="vdo-btn-wrap">  <a href="'.$button_two_url.'" class="video-play mfp-iframe">'.$icon_markup.' '.$button_two_text.'</a> </div>';
        }
        return <<<HTML
  <div class="header-area style-04 header-bg-04"  data-padding-top="{$padding_top}" data-padding-bottom="{$padding_bottom}"
        {$background_image}
        >
            <div class="container">

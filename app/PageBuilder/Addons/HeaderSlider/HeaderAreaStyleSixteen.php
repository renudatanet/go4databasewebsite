...
class HeaderAreaStyleSixteen extends PageBuilderBase
{
    use RepeaterHelper;
    /**
     * preview_image
    ...
    {
        return 'header-area-style-16.png';
    }
    ...
    private function render_slider_markup(int $index = null): string
    {
        ...
        if (!empty($button_one_text) && !empty($button_one_url)){
            $button_markup = '<div class="btn-wrapper desktop-left padding-top-30"> <a href="'.$button_one_url.'" class="boxed-btn">'.$button_one_text.'</a>  </div>';
        }

        $video_button_markup = '';
        if ( !empty($button_two_url)){
            $icon_markup =  '<i class="fas fa-play"></i>' ;
            $video_button_markup = '<div class="vdo-btn-wrapper">  <a href="'.$button_two_url.'" class="video-play-btn mfp-iframe">'.$icon_markup.'</a> </div>';
        }
        $settings = $this->get_settings();
        $padding_top = SanitizeInput::esc_html($settings['padding_top']);
        $padding_bottom = SanitizeInput::esc_html($settings['padding_bottom']);
        return <<<HTML
  <div class="header-area header-bg-02" data-padding-top="{$padding_top}" data-padding-bottom="{$padding_bottom}"
    {$background_image}
    >
        <div class="container">
            <div class="row">

...
class PricePlanGridStyleOne extends PageBuilderBase
{
    ...
    {
       return 'price-plan/slider-01.png';
    }
    ...
    public function frontend_render()
    {
        ...
        $price_plan = $price_plan->orderBy($order_by,$order);
        if(!empty($items)){
            $price_plan = $price_plan->paginate($items);
        }else{
            $price_plan = $price_plan->get();
        }


        $pagination_markup = '';
        if (!empty($pagination_status) && !empty($items)){
            $pagination_markup = '<div class="col-lg-12"><div class="pagination-wrapper '.$pagination_alignment.'">'.$price_plan->links().'</div></div>';
        }

        $category_markup = '';
        foreach ($price_plan as $plan){
            $highlight = $plan->highlight ? 'style-02 active' : '';
            $title = $plan->title;
...
<div class="col-md-6 {$columns}">
    <div class="single-price-plan-01 {$highlight} margin-bottom-30">
...
HTML;
        ...
        return <<<HTML
<div class="pricing-plan-area bg-liteblue price-inner" data-padding-top="{$padding_top}" data-padding-bottom="{$padding_bottom}" {$background_color}>
    <div class="container">
        {$section_title_markup}
        <div class="row">
             {$category_markup}

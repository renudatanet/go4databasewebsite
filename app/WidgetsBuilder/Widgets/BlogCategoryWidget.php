...
class BlogCategoryWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        if (!empty($widget_title)) {
            $output .= '<p class="widget-title style-01">' . purify_html($widget_title) . '</p>';
        }
        $output .= '<ul>';
        foreach ($blog_posts as $post) {
            $output .= '<li><a href="' . route('frontend.blog.category', ['any' => Str::slug(str_replace('&', 'and', purify_html($post->name)))]) . '"> <i class="fas fa-chevron-'.$icon_class.'"></i>' . purify_html($post->name) . '</a></li>';
        }
        $output .= '</ul>';
        $output .= '</div>';

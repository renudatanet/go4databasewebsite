...
class EventCategoryWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog_posts = EventsCategory::where(['status' => 'publish','lang' => LanguageHelper::user_lang_slug()])->orderBy('id','desc')->get();
        if (!empty($post_items)){
        ...
        foreach ($blog_posts as $post) {
            $output .= '<li><a href="' . route('frontend.events.category', ['id' => $post->id,'any' => Str::slug(purify_html($post->title))]) . '"> ' . purify_html($post->title) . '</a></li>';
        }
        $output .= '</ul>';
        $output .= '</div>';

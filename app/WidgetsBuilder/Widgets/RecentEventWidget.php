...
class RecentEventWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog_posts = Events::where(['status' => 'publish','lang' => LanguageHelper::user_lang_slug()])->get();
        if (!empty($post_items)){
        ...
        foreach ($blog_posts as $post) {
            $output .= '<li class="single-recent-post-item">
                    ...
                    <div class="content">
                        <h4 class="title"><a href="' . route('frontend.events.single',['slug' => $post->slug ]) . '">' . purify_html($post->title) . '</a></h4>

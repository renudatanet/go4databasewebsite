...
class RecentKnowledgebasePostWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog_posts = Knowledgebase::where(['status' => 'publish','lang' => LanguageHelper::user_lang_slug()])->get();
        if (!empty($post_items)){
        ...
        foreach ($blog_posts as $post) {
            $output .= '<li class="single-recent-post-item">
                    <div class="content">
                        <h4 class="title"><a href="' . route('frontend.knowledgebase.single',['slug' => $post->slug ]) . '"><i class="far fa-file-alt base-color"></i> ' . purify_html($post->title) . '</a></h4>
    ...
    {
        return __('Recent Knowledgebase Post');
    }
}
...
@section('site-title')
    {{__('Product Order User Template')}}
@endsection
@section('style')
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="header-wrapp d-flex justify-content-between">
                            <h4 class="header-title">{{__('Product Order User Template')}}</h4>
                            <a class="btn btn-info" href="{{route('admin.email.template.all')}}">{{__('All Email Templates')}}</a>
                        </div>
                        <form action="{{route('admin.email.template.product.order.mail.user')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <nav>
                                ...
                                @foreach($all_languages as $key => $lang)
                                    <div class="tab-pane fade @if($key == 0) show active @endif" id="nav-home-{{$lang->slug}}" role="tabpanel" aria-labelledby="nav-home-tab">
                                        <div class="form-group">
                                            <label for="product_order_user_mail_{{$lang->slug}}_subject">{{__('Subject')}}</label>
                                            <input type="text" name="product_order_user_mail_{{$lang->slug}}_subject"  class="form-control" value="{{get_static_option('product_order_user_mail_'.$lang->slug.'_subject')}}">
                                        </div>
                                        <div class="form-group">
                                            <label for="product_order_user_mail_{{$lang->slug}}_message">{{__('Message')}}</label>
                                            <input type="hidden" name="product_order_user_mail_{{$lang->slug}}_message"  class="form-control" value="{{get_static_option('product_order_user_mail_'.$lang->slug.'_message')}}" >
                                            <div class="summernote" data-content='{{get_static_option('product_order_user_mail_'.$lang->slug.'_message')}}'></div>

                      ...
                      <x-error-msg/>
                        <form action="{{route('admin.general.page.settings')}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    @php
                                        $pages_list = ['video_gallery','about','service','work','list','team','faq','author','price_plan','blog','contact','career_with_us','events','knowledgebase','donation','product','testimonial','feedback','clients_feedback','image_gallery','donor','appointment','quote','courses','support_ticket','case_study'];
                                    @endphp
                                                            ...
                                                            <div id="{{$page}}_page_content_{{$lang->slug}}" class="collapse"  data-parent="#accordion-{{$lang->slug}}">
                                                                <div class="card-body">
                                                                    ...
                                                                    <div class="form-group margin-top-20">
                                                                        <label for="{{$page}}_page_{{$lang->slug}}_meta_tags">{{__('Title')}}</label>
                                                                        <input type="text" name="{{$page}}_page_{{$lang->slug}}_meta_tags"  class="form-control meta_title" value="{{get_static_option($page.'_page_'.$lang->slug.'_meta_tags')}}"  maxlength="60">
                                                                        <span class="titleLengthMessage"></span>
                                                                    </div>
                                                                    <div class="form-group">
                                                                        <label for="about_page_{{$lang->slug}}_meta_description">{{__('Meta Description')}}</label>
                                                                        <textarea name="{{$page}}_page_{{$lang->slug}}_meta_description"  class="form-control meta_description" rows="5" maxlength="160">{{get_static_option($page.'_page_'.$lang->slug.'_meta_description')}}</textarea>
                                                                        <span class="descriptionLengthMessage"></span>
                                                                    </div>
                                                                    <div class="form-group">
                                                                        <label for="about_page_{{$lang->slug}}_schema_code">{{__('Schema')}}</label>
                                                                        <textarea name="{{$page}}_page_{{$lang->slug}}_schema_code"  class="form-control" rows="5" >{{get_static_option($page.'_page_'.$lang->slug.'_schema_code')}}</textarea>
                                                                    </div>
                                                                    <x-media-upload :title="__('Og Meta Image')" :name="$page.'_page_'.$lang->slug.'_meta_image'" :dimentions="'1200x1200'"/>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        @endforeach

                ...
                <div class="card">
                    <div class="card-body">
                       <div class="header-wrap d-flex align-content-center justify-content-between">
                           <div class="left-wrap">
                               <h4 class="header-title">{{__('All Author Items')}}</h4>
                           ...
                           <div class="right-wrap">
                               <a href="{{route('admin.author.new')}}" class="btn btn-primary">{{__('Create New Author')}}</a>
                           </div>
                       </div>
...
<table id="blogTable" class="table table-default">
                                      ...
                                      <tbody>
                                        @foreach($all_author as $key => $data)
                                            <tr>
                                                <td>
                                                ...
                                                <td>
                                                    <x-delete-popover :url="route('admin.author.delete',$data->id)"/>
                                                    <a href="{{route('admin.author.edit',$data->id)}}" class="btn btn-xs btn-primary btn-xs mb-3 mr-1 work_edit_btn">
                                                        <i class="ti-pencil"></i>
                                                    </a>
                                                    <br>
                                                    <a target="_blank" href="{{route('frontend.author.single',$data->slug)}}" class="btn btn-xs btn-info btn-sm mb-3 mr-1">
                                                        <i class="ti-eye"></i>
                                                    </a>
                                                    <form action="{{route('admin.author.clone')}}" method="post" style="display: inline-block">
                                                        @csrf
        ...
        $(document).ready(function() {
            $(document).on('click','#bulk_delete_btn',function (e) {
                ...
                if(allIds != '' && bulkOption == 'delete'){
                    ...
                    $.ajax({
                        'type' : "POST",
                        'url' : "{{route('admin.author.bulk.action')}}",
                        'data' : {
                            _token: "{{csrf_token()}}",
                            ids: allIds
                        },

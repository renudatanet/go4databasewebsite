...
let table = $('#blogTable').DataTable({
    ...
    columns: [
        ...
        { data: 'updated_date', searchable: false },
...
$('#previewBtn').on('click', function() {
    ...
    $.ajax({
        url: "{{ route('admin.blog.replace.preview') }}",
        method: "POST",
        data: formData,
...
$('#replaceUrlForm').on('submit', function(e) {
    ...
    if (!confirm('Are you sure? This will update ALL blogs!')) {
        return;
    }

    $.ajax({
        url: "{{ route('admin.blog.replace.url') }}",
        method: "POST",
        data: formData,
        success: function(res) {
            alert(res.message);
...
$(document).on('click', '.delete-blog', function () {
    ...
    $.ajax({
        url: "{{ url('admin-home/blog/delete') }}/" + id,
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}"
        },

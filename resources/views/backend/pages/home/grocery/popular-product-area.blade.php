    ...
    <div class="col-lg-12 col-ml-12 padding-bottom-30">
        <div class="row">
            ...
            <div class="col-lg-12 mt-t">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title">{{__('Popular Items Area Settings')}}</h4>

                        <form action="{{route('admin.home18.popular.item')}}" method="post" enctype="multipart/form-data">
                            @csrf
                                ...
                                @foreach($all_languages as $key => $lang)
                                    <div class="tab-pane fade @if($default_lang == $key) show active @endif" id="nav-home-{{$lang->slug}}" role="tabpanel" aria-labelledby="nav-home-tab">
                                        <div class="form-group">
                                            <label for="grocery_home_page_{{$lang}}_featured_product_area_subtitle">{{__('Subtitle')}}</label>
                                            <input type="text" name="grocery_home_page_{{$lang->slug}}_featured_product_area_subtitle" value="{{get_static_option('grocery_home_page_'.$lang->slug.'_featured_product_area_subtitle')}}" class="form-control" >
                                        </div>
                                        <div class="form-group">
                                            <label for="grocery_home_page_{{$lang}}_featured_product_area_title">{{__('Title')}}</label>
                                            <input type="text" name="grocery_home_page_{{$lang->slug}}_featured_product_area_title" value="{{get_static_option('grocery_home_page_'.$lang->slug.'_featured_product_area_title')}}" class="form-control" >
                                        </div>
                                        <div class="form-group">

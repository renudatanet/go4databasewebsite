...
<a href="{{ route('admin.page.scan.results') }}" class="btn btn-primary">
    View Results
</a>
</div>
                                        <table  id="pageTable" class="table table-default">
                                        <thead>
                                        <th class="no-sort">
...
let table = $('#pageTable').DataTable({
    ...
    columns: [
            ...
            { data: 'created_at', searchable: false },  // ✅ ONLY this searchable
...
$('#previewBtn').on('click', function() {
    ...
    $.ajax({
        url: "{{ route('admin.page.replace.preview') }}",
        method: "POST",
        data: formData,
...
$('#replaceUrlForm').on('submit', function(e) {
    ...
    if (!confirm('Are you sure? This will update ALL pages!')) {
        return;
    }

    $.ajax({
        url: "{{ route('admin.page.replace.url') }}",
        method: "POST",
        data: formData,
        success: function(res) {
            alert(res.message);
...
$("#startScanBtn").click(function () {
    ...
    // Start scan
    $.get("{{ route('admin.page.scan.full') }}", function (res) {
...
function checkProgress() {
    ...
    $.get("{{ route('admin.scan.progress') }}", function (res) {
        ...
        if (res.running == 1) {
            ...
            // 🔥 AUTO REDIRECT
            setTimeout(function () {
                window.location.href = "{{ route('admin.page.scan.results') }}";
            }, 2000);
        }
    });
}
...
$(document).on('click', '.delete-blog', function () {
    ...
    $.ajax({
        url: "{{ url('admin-home/page/delete') }}/" + id,
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}"
        },

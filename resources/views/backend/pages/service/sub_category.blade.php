                                    ...
                                    <div class="table-wrap table-responsive">
                                        ...
                                        @foreach($category as $data)
                                            <tr>
                                                ...
                                                <td>
                                                     <x-delete-popover :url="route('admin.service.sub_category.delete',$data->id)"/>
                                                    <a href="#"
                                                       data-toggle="modal"
                                                       ...
                                                       data-image="{{$data->img_icon}}"
                                                        data-metatag="{{$data->meta_tags}}"
                                                       data-schemacode="{{$data->schema_code}}"
            ...
            <div class="col-lg-6 mt-5">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{route('admin.service.sub_category')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="form-group">
                            <label for="edit_language">{{__('Category')}}</label>
...
<div class="form-group business-title-field" style="display:none;">
    <label>{{__('Title')}}</label>
    <input type="text" name="title" class="form-control">
</div>
                            <div class="form-group">
                                <label for="meta_tags">{{__('Meta Title')}}</label>
    ...
    <div class="modal fade" id="category_edit_modal" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{route('admin.service.sub_category.update')}}"  method="post">
    ...
    <div class="d-flex">
        <label class="mr-3">
            <input type="radio" name="edit_service_type" value="0" >
            Professional
        </label>

        <label>
            <input type="radio" name="edit_service_type" value="1" >
            Business
        </label>
    </div>
</div>

<div class="form-group edit-business-title-field" style="display:none;">
    <label>{{__('Title')}}</label>
    <input type="text" name="title" id="edit_title" class="form-control" >
</div>
                        <div class="form-group">
                            <label for="edit_meta_tags">{{__('Meta Title')}}</label>
                            <input type="text" name="meta_tags"  class="form-control" value="" id="edit_meta_tags" maxlength="60">
                            <span id="titleLengthMessage"></span>
                        </div>
                        <div class="form-group">
                            <label for="edit_meta_description">{{__('Meta Description')}}</label>
                            <textarea name="meta_description"  class="form-control" rows="5" id="edit_meta_description"  maxlength="160"></textarea>
                            <span id="descriptionLengthMessage"></span>
                        </div>
                        
                        <div class="form-group">
                            <label for="edit_schema_code">{{__('Schema')}}</label>
                            <textarea name="schema_code"  class="form-control" rows="5" id="edit_schema_code"></textarea>
                        </div>
                        <div class="form-group">
...
@section('script')
    ...
    $('input[name="service_type"]').on('change', function () {
        ...
        if ($(this).val() == '1') {
            $('.business-title-field').show();
        } else {
            $('.business-title-field').hide();
        }

    });

    // EDIT FORM
    $('input[name="edit_service_type"]').on('change', function () {

        if ($(this).val() == '1') {
            $('.edit-business-title-field').show();
        } else {
            $('.edit-business-title-field').hide();
        }
        ...
        $(document).ready(function () {
            ...
            $(document).on('click','#bulk_delete_btn',function (e) {
                ...
                if(allIds != '' && bulkOption == 'delete'){
                    ...
                    $.ajax({
                        'type' : "POST",
                        'url' : "{{route('admin.service.sub_category.bulk.action')}}",
                        'data' : {
                            _token: "{{csrf_token()}}",
                            ids: allIds
                        },
...
$(document).on('click','.category_edit_btn',function(){
    ...
    var serviceType = el.data('service_type');
    var title = el.data('title');

    var id = el.data('id');
    var name = el.data('name');

    var metatag = el.data('metatag');
    var metadescription = el.data('metadescription');
    var schemacode = el.data('schemacode');
    ...
    // TYPE
    modal.find('input[name="edit_service_type"][value="'+serviceType+'"]').prop('checked', true);

    // TITLE
    modal.find('#edit_title').val(title);

    if(serviceType == 1){
        modal.find('.edit-business-title-field').show();
    }else{
        modal.find('.edit-business-title-field').hide();
    }

    modal.find('#subcategory_id').val(id);

    modal.find('#edit_language option[value="'+el.data('lang')+'"]').prop('selected', true);

    modal.find('#edit_status option[value="'+status+'"]').prop('selected', true);

    modal.find('#edit_name').val(name);

    modal.find('#edit_meta_tags').val(metatag);

    modal.find('#edit_meta_description').val(metadescription);

    modal.find('#edit_schema_code').val(schemacode);

    modal.find('select[name="icon_type"] option[value="'+iconType+'"]').prop('selected', true);

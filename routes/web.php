...
Route::get('/test-mail', function () {
    ...
    Mail::raw('Test Email', function ($message) {
        $message->to('renurana501@gmail.com')
                ->subject('SMTP Test');
    });
...
Route::get('/pages_sitemap.xml', [SitemapController::class, 'pages']);
Route::get('/b2b_sitemap.xml', [SitemapController::class, 'b2b']);
Route::get('/b2c_sitemap.xml', [SitemapController::class, 'b2c']);
Route::get('/list_sitemap.xml', [SitemapController::class, 'list']);
Route::get('/blog_sitemap.xml', [SitemapController::class, 'blog']);
Route::get('/case_study_sitemap.xml', [SitemapController::class, 'case_study']);
Route::get('/career_sitemap.xml', [SitemapController::class, 'career']);
...
//rss feed route
Route::feeds();
Route::any('/product-category/{any}', function () {
    return response()
        ->view('errors.410', [], 410);
})->where('any', '.*');
Route::any('/b2b/category/{any}', function () {
    return response()
        ->view('errors.410', [], 410);
})->where('any', '.*');
Route::any('/blog-tags/{any}', function () {
    return response()
        ->view('errors.410', [], 410);
})->where('any', '.*');
...
Route::get('/{year}/{month}/{day}/{slug}', function () {
    return response()
        ->view('errors.410', [], 410);
})->where([
    'year'  => '[0-9]{4}',
...
Route::any('/country/{any}', function () {
     return response()
        ->view('errors.410', [], 410);
})->where('any', '.*');

Route::any('/service/category/{any}', function () {
     return response()
        ->view('errors.410', [], 410);
})->where('any', '.*');
...
// Route::get('/blog-tags/{any}', function () {
//     return redirect('/blog', 301); // or your preferred destination
// })->where('any', '.*');
        //courses
...
Route::group(['middleware' => ['setlang:frontend', 'globalVariable', 'maintains_mode', 'moduleCheck:job_module_status']], function () {
   ...
   // Route::get( $career_with_us_page_slug.'-category/{id}/{any}', 'FrontendController@jobs_category')->name('frontend.jobs.category');
    ...
    Route::get( $career_with_us_page_slug.'/{any}', 'FrontendController@jobs_category')->name('frontend.jobs.category');
...
Route::group(['middleware' => ['setlang:frontend', 'globalVariable', 'maintains_mode', 'HtmlMinifier']], function () {
    ...
    $case_study_page_slug = get_static_option('case_study_page_slug') ?? 'case-study';
    ...
    $list_page_slug = get_static_option('list_page_slug') ?? 'list';
    $author_page_slug='author';
...
// List Page
 Route::get($list_page_slug, 'FrontendController@list')->name('frontend.b2c');
  Route::get('list/{service_slug}', 'FrontendController@list_single')->where('service_slug', '.*')
    ->name('frontend.list.single');
       
      Route::post('/lead-submit',  'FrontendController@submit')->name('service-lead.submit');
      Route::post('/sample-submit',  'FrontendController@submitSample')->name('sample-lead.submit');
    /*--------------------------------------
    ...
    Route::get('/' . $author_page_slug, 'FrontendController@author_page')->name('frontend.author');
    ...
    /*--------------------------------------
     ...
     ---------------------------------------*/
        ...
        Route::get( $work_page_slug.'/testapi/{slug}', 'FrontendController@testapi')->name('frontend.work.testapi');
       // Route::get( $work_page_slug.'/category/{id}/{any?}', 'FrontendController@category_wise_works_page')->name('frontend.works.category');

     /*--------------------------------------
         FRONTEND: CASE STUDY ROUTES
     ---------------------------------------*/
        Route::get($case_study_page_slug, 'FrontendController@case_study_page')->name('frontend.case-study');
        Route::get( $case_study_page_slug.'/{slug}', 'FrontendController@case_study_single_page')->name('frontend.case-study.single');
        Route::get( $case_study_page_slug.'/category/{id}/{any?}', 'FrontendController@category_wise_works_page')->name('frontend.case-study.category');
    ...
    ---------------------------------------*/
        ...
        // Route::get($blog_page_slug.'/author', 'FrontendController@author_list')->name('blog.author');
        Route::get($blog_page_slug.'/author/{author}', 'FrontendController@author_single_page')->name('blog.author');
       ...
       // Route::get($blog_page_slug.'/author/{author}', 'FrontendController@blog_single_page')->name('blog.author');
         
 /*---------------------- ----------------
        FRONTEND: AUTHORS ROUTES
    ---------------------------------------*/

        Route::get( $author_page_slug.'/{slug}', 'FrontendController@author_single_page')->name('frontend.author.single');

        //Route::get( $blog_page_slug.'-category/{id}/{any}', 'FrontendController@category_wise_blog_page')->name('frontend.blog.category');
        Route::get( $blog_page_slug.'/{any}', 'FrontendController@category_wise_blog_page')->name('frontend.blog.category');
      //  Route::get( $blog_page_slug.'-tags/{name}', 'FrontendController@tags_wise_blog_page')->name('frontend.blog.tags.page');
  Route::get('/leads', 'FrontendController@showLeads')->name('showLeads');
    /*------------------------------------
        FRONTEND: ROUTES
    ------------------------------------*/
...
// Local testing only, see WebsiteLeadsLocalProxyController.
